...
class SiteController extends Controller
{
	...
	public function beforeAction($action){
		...
		if($model->load(Yii::$app->request->post()) && $model->validate()){
			$q = Html::encode($model->q); 	//тоже самое, что и $q = htmlspecialchars($q);
			return $this->redirect(Yii::$app->urlManager->createUrl(["site/search", 'q' => $q]));
		}
		return true;	//на случай, если не было отправлено формы
	}
	...
	//если в названии большая буква => в url будет дефис
	public function actionAddsite(){
		$model = new SiteForm();
		
		//если загружена и прошла проверку
		if($model->load(Yii::$app->request->post()) && $model->validate()){
			$site = new Sites();
